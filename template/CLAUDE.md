...
- This repo uses `agentic/pm_flow/pm_flow.sh` for Claude PM review preparation and transcript logging.
- Use `agentic/pm_flow/<project>/project_state/` as the stable repo-local continuation layer for plan files and current-run pointers.
...
- Use `agentic/pm_flow/net_exec.sh` or other stable wrappers before switching to ad hoc command shapes.
- If network access fails in a sandboxed command, treat that as a sandbox restriction first.
- Every cycle must use the task contract in `agentic/pm_flow/<project>/task_contract.md`.
