...
artifact contract. A legacy brief requires Objective, Scope, Priority, Owned
...
The resulting coverage finding belongs to the brief.

## Boundaries

`boundaries` means a section artifact contains a path-like inline-code reference
outside the paths declared by its brief. The section's four sibling durable
filenames and dependency handoffs declared by the brief are allowed references.
An inline-code reference to another live section key is allowed only when that
key is declared under Dependencies. The project plan has portfolio scope and is
not checked against a section boundary.
