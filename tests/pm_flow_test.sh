...
#
# The suite drives its fixtures through the wheel-installed `pm-flow`, never
# through a copy of pm_flow.sh sitting in the fixture, because there is no such
# copy any more: install.sh writes project data and the engine ships inside the
# package. Running the checkout's scripts instead would pass while the whole
# claim of this section was false.
#
# The build is the accepted offline one. A build venv is populated from pinned,
# hashed, platform-independent wheels with the package index switched off, and a
# separate clean runtime venv holds nothing but the wheel that build produced -
# so a build-time import can never answer for something the shipped artifact is
# supposed to provide.
...
# An inherited VIRTUAL_ENV, PYTHONPATH, PIP_* or UV_* setting would let a
# different interpreter, a different index or a cache this suite never wrote
# answer for the artifact it claims to be proving.
...
# The caller's shell startup files, removed for the same reason. This suite is
...
# directory lands *ahead* of the stub CLIs the fixtures inject, and a real
# vendor binary answers a dispatch this suite believes it stubbed. Pointing
# ZDOTDIR at an empty directory under TEST_ROOT leaves the exported PATH as
# written and keeps the caller's shell configuration out of the artifact proof.
...
: > "$BUILD_LOG"
build_step "build venv creation failed" python3 -m venv "$BUILD_VENV"
build_step "the locked build requirements did not install offline from $WHEELHOUSE" \
  "$BUILD_VENV/bin/python" -m pip install --no-index --find-links "$WHEELHOUSE" \
  --disable-pip-version-check --no-input --quiet --require-hashes \
...
build_step "the offline wheel build failed" \
  "$BUILD_VENV/bin/python" -m pip wheel --no-index --no-build-isolation --no-deps \
  --disable-pip-version-check --no-input --wheel-dir "$DIST" "$REPO_ROOT"

WHEEL="$(find "$DIST" -maxdepth 1 -type f -name 'pm_flow-*.whl' | sort | /usr/bin/head -n 1)"
[[ -n "$WHEEL" ]] || fail "the offline build produced no pm_flow wheel"
build_step "runtime venv creation failed" python3 -m venv "$VENV"
build_step "installing the built wheel into the runtime venv failed" \
  "$VENV/bin/python" -m pip install --quiet --no-index --no-deps \
...
# Where the engine actually came from. Every direct invocation of an engine
# script below - the dispatcher, the access hook, the driver's own functions -
# is resolved from here, so none of them can silently read the checkout.
ENGINE="$("$PM_FLOW" version | awk '/^ *engine:/ {sub(/^ *engine: */, ""); print; exit}')"
[[ -n "$ENGINE" && -f "$ENGINE/pm_flow.sh" ]] || fail "pm-flow reported no packaged engine"
case "$ENGINE" in
  "$VENV"/*) ;;
  *) fail "the engine is not the installed package's: $ENGINE" ;;
esac
assert_not_contains "$ENGINE" "$REPO_ROOT" "the engine resolves outside the checkout"
...
PM=(env PM_FLOW_REPO_ROOT="$FIXTURE_REPO" "$PM_FLOW")
FIXTURE_FLOW="$FIXTURE_REPO/.agentic/pm_flow"
PROJECT_DIR="$FIXTURE_REPO/.agentic/pm_flow/fixture-repo"

# What used to be asserted here is now the opposite claim: the installer ships
# no engine, so none of these may exist in the repository it wrote.
assert_no_copied_engine() {
  local flow="$1" label="$2" name
  for name in pm_flow.sh net_exec.sh agent_exec.sh access_hook.sh fetch.sh \
              heartbeat.sh driver.zsh catalog.py cost.py store.py telemetry.py \
              trace_export.py watch.py upgrade.py README.md; do
    [[ ! -e "$flow/$name" ]] || fail "$label: a copied engine file exists: $name"
  done
  for name in roles domains tasks project tests .pm-flow; do
    [[ ! -e "$flow/$name" ]] || fail "$label: a packaged resource exists: $name"
  done
  local stray
  stray="$(find "$flow/.." \( -name MANIFEST -o -name upgrade.py \) -print 2>/dev/null)"
  [[ -z "$stray" ]] || fail "$label: an install record survives:"$'\n'"$stray"
}
assert_no_copied_engine "$FIXTURE_FLOW" "a fresh install"
assert_file_contains \
  "$FIXTURE_REPO/CLAUDE.md" \
...
# A remote install that cannot fetch every template must not leave the
# repository half-written. There is no installed engine left to truncate, so
# what is at stake now is the project's own files - its contract and its plan -
# and everything is fetched before anything is written for exactly that reason.
contract_before_failed_install="$(/bin/cat "$PROJECT_DIR/task_contract.md")"
plan_before_failed_install="$(/bin/cat "$PROJECT_DIR/project_state/plan.md")"
...
expect_failure \
  "late failed remote install" \
  "$REPO_ROOT/install.sh" \
  "$FIXTURE_REPO" \
  --name "Fixture Project" \
  --repo-raw-base "file://$partial_remote_source"
[[ "$contract_before_failed_install" == "$(/bin/cat "$PROJECT_DIR/task_contract.md")" ]] || \
  fail "a failed template fetch rewrote the project's contract"
[[ "$plan_before_failed_install" == "$(/bin/cat "$PROJECT_DIR/project_state/plan.md")" ]] || \
  fail "a failed template fetch rewrote the project plan"
assert_no_copied_engine "$FIXTURE_FLOW" "a failed remote install"
...
MOVE_PM=(env PM_FLOW_REPO_ROOT="$MOVE_SOURCE" "$PM_FLOW")
...
MOVED_PM=(env PM_FLOW_REPO_ROOT="$MOVE_DESTINATION" "$PM_FLOW")
...
ROLE_PM=(env PM_FLOW_REPO_ROOT="$ROLE_REPO" "$PM_FLOW")
ROLE_FLOW="$ROLE_REPO/.agentic/pm_flow"
# The dispatcher comes from the package. It is invoked as `zsh -f <path>`
# because package data carries no execute bit, and it is handed the three roots
# `pm-flow` would otherwise have exported for it.
AGENT_EXEC=(env PM_FLOW_ENGINE_ROOT="$ENGINE" PM_FLOW_FLOW_DIR="$ROLE_FLOW" \
  PM_FLOW_REPO_ROOT="$ROLE_REPO" zsh -f "$ENGINE/agent_exec.sh")

assert_no_copied_engine "$ROLE_FLOW" "a domain install"
[[ -f "$ENGINE/agent_exec.sh" ]] || fail "the installed package ships no dispatcher"
...
generic_prompt="$(env PM_FLOW_REPO_ROOT="$generic_repo" "$PM_FLOW" role-prompt cpo)"
...
migration_prompt="$(env PM_FLOW_REPO_ROOT="$migration_repo" "$PM_FLOW" role-prompt pm)"
...
MULTI_PM=(env PM_FLOW_REPO_ROOT="$multi_repo" "$PM_FLOW")
...
migration_dispatch="$(env PM_FLOW_PROJECT=migration PM_FLOW_ENGINE_ROOT="$ENGINE" \
  PM_FLOW_FLOW_DIR="$MULTI_FLOW" PM_FLOW_REPO_ROOT="$multi_repo" \
  zsh -f "$ENGINE/agent_exec.sh" pm \
...
DRIVER_PM=(env PM_FLOW_REPO_ROOT="$DRIVER_REPO" "$PM_FLOW")
...
SCHED_PM=(env PM_FLOW_REPO_ROOT="$SCHED_REPO" "$PM_FLOW")
...
ACCESS_HOOK="$ENGINE/access_hook.sh"
[[ -f "$ACCESS_HOOK" ]] || fail "the installed package ships no access hook"
[[ ! -e "$FIXTURE_FLOW/access_hook.sh" ]] || \
  fail "the access hook was copied into the repository"
...
access_probe() {
  printf '%s' "$1" | PM_FLOW_ACCESS_LOG="$ACCESS_LOG" PM_FLOW_ACCESS_ROLE=developer \
    ...
    PM_FLOW_ACCESS_WORK_ROOT="$FIXTURE_REPO" zsh -f "$ACCESS_HOOK"
}
...
WT_PM=(env PM_FLOW_REPO_ROOT="$WT_REPO" "$PM_FLOW")
...
( eval "$(sed -n '/^sync_section_worktree() {/,/^}/p' "$ENGINE/driver.zsh")"
...
DECOMP_PM=(env PM_FLOW_REPO_ROOT="$DECOMP_REPO" "$PM_FLOW")
...
assert_not_contains "$("${DECOMP_PM[@]}" status)" "widget" "an empty project has no sections yet"
decomp_code=0
decomp_run="$(PM_DONE_FLAG="$TEST_ROOT/decomp.flag" PATH="$TEST_ROOT/decomp-bin:$PATH" \
  "${DECOMP_PM[@]}" run --max-ticks 20 2>&1)" || decomp_code=$?
...
decomp_status="$("${DECOMP_PM[@]}" status)"
