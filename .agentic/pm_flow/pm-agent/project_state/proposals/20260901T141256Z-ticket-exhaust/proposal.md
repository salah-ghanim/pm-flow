Probes done — no existing coverage on `main`, and the plan explicitly promises this capability. Cutting the section.

## Assessment

The request asks for a one-way GitHub Issues exhaust (issue per section, comment per accepted cycle, close on done) plus inbound `init-section --from-issue`, with the files staying authoritative. It serves the plan's completion criterion directly: *"A section is visible in an external ticket tracker, GitHub Issues first: created with its objective, updated on every accepted cycle, closed on completion. The tracker is a view; the files stay the truth."* — no invention needed.

Probes run:

- `grep -rli ticket template/ src/ tests/` — no hits; nothing on `main` does this.
- `grep -n "gh " pm_flow.sh driver.zsh` — only false hits on the word "through"; no `gh` CLI usage exists.
- `sections.md` — no live or cancelled section covers it; `a2a-binding` and `repo-hooks` were cut for unrelated reasons.
- `sections/boundary-schema/brief.md` — anticipates this exactly: its schemas are "the boundary schemas the future ticket-integration section will consume," and its non-goals say "The ticket-tracker section (GitHub Issues first) — it depends on this; do not start it here."
- `pm_flow.sh:53,1667,1990` — `init-section` lives in `pm_flow.sh`, which `boundary-schema` owns live; the sync verb and `--from-issue` flag must land after that section closes. `driver.zsh` (post-accept hook) is `outcome-record`'s; `install.sh` registration is `real-install`'s. All three are handled as ordering/hand-over constraints; only `boundary-schema` is a true dependency, because sync consumes its validated export.
- `fetch.sh` header — confirms the credential-isolation pattern the request cites.

One conflict: the owner marked the request nice-to-have, but the tracker bullet sits under "What must be true when this is done," and nothing else can meet it. Priority follows the plan (must-have); the divergence is an open question for the owner.
...
### Owned paths
- template/.agentic/pm_flow/ticket_sync.*
...
CUT — the request maps one-to-one onto the plan's unserved ticket-tracker completion criterion, nothing on `main` or in any live section covers it, and `boundary-schema` already reserved the export interface for exactly this consumer.
