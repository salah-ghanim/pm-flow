# Database state, replayable timelines and workflow experiments Task Contract

Primary mission:

- Make project coordination authoritative in SQLite as an ordered transaction log, execute versioned task graphs with bounded handovers, replay or fork any checkpoint to compare agents, instructions, tasks and topologies in parallel under independent evaluation, and migrate existing projects without loss.
...
## OpenTelemetry acceptance — owner requirement, 2026-09-06

OpenTelemetry compatibility and accurate token tracking are mandatory for the
current engine and every new execution, database and visual workflow path.

- Correlate task and attempt identities, agent/CLI/model/effort, exact prompt
  revision and source commit with durable usage records and exported spans.
- Preserve provider-reported input, output, cache and reasoning usage where
  available. Distinguish unavailable fields from zero; do not double-count
  retries, cumulative usage events or repeated exports.
- Cover failures, rejections, retries, interruptions, resume and concurrent
  branches as well as success. Usage survives restart and collector outage.
- Reuse the existing store and telemetry/export implementation. UI counters
  consume the same records; they are not an independent accounting system.
- An affected section must prove its integration in a real OpenTelemetry
  backend, reconciling representative real provider usage with stored and
  exported token totals. Automated fixture tests supplement that evidence.
  A missing supported-provider probe remains explicitly unproven.
- Never include credentials in traces. Prompt identity and provenance are
  required; recording raw prompt content is a separate controlled choice.

This requirement does not authorize a section to edit another section's owned
paths. Report an integration dependency through its bounded handoff and have
the product officer settle ownership before assignment.
...
- this checkout on main; `docs/database-workflows-spec.md` at `9236bde` is the outcome contract, and project_state/plan.md names what must be true when it is done
